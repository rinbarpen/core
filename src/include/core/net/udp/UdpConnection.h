...
public:
  SHARED_REG(UdpConnection);
  using ReadCallback = std::function<bool(UdpConnection::ptr, BufferReader&)>;
  using DisconnectCallback = std::function<void(UdpConnection::ptr)>;
  using CloseCallback = std::function<void(UdpConnection::ptr)>;
  using ErrorCallback = std::function<void(UdpConnection::ptr)>;

  UdpConnection(TaskScheduler* task_scheduler, sockfd_t fd);
