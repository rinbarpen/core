#include <core/multimedia/capture/WASAPIPlayer.h>
...
bool WASAPIPlayer::start() {
  ...
  if (FAILED(hr)) {
    ILOG_ERROR(g_multimedia_capture_logger) << "[WASAPIPlayer] Failed to start audio client.";
    return false;
  }
...
bool WASAPIPlayer::stop() {
  ...
  if (enabled_) {
    ...
    if (FAILED(hr)) {
      ILOG_ERROR(g_multimedia_capture_logger) << "[WASAPIPlayer] Failed to stop audio client.";
      return false;
    }
  }
...
bool WASAPIPlayer::play() {
  ...
  if (FAILED(hr)) {
    ILOG_ERROR(g_multimedia_capture_logger) << "[WASAPIPlayer] Failed to get current padding.";
    return false;
  }
  ...
  if (numFramesAvailable > 0) {
    ...
    if (FAILED(hr)) {
      ILOG_ERROR(g_multimedia_capture_logger) << "[WASAPIPlayer] Audio render client failed to get buffer.";
      return false;
    }
    ...
    if (FAILED(hr)) {
      ILOG_ERROR(g_multimedia_capture_logger) << "[WASAPIPlayer] Audio render client failed to release buffer.";
      return false;
    }
  }
  else {
