...
LY_NAMESPACE_BEGIN
NAMESPACE_BEGIN(net)
EventLoop::EventLoop(size_t nThreads) : capacity_(nThreads > 0 ? nThreads : 1) {
  this->start();
}
EventLoop::~EventLoop() {
  this->stop();
...
TaskScheduler::ptr EventLoop::getTaskScheduler() {
  Mutex::lock locker(mutex_);
  auto task_scheduler = task_schedulers_[current_task_scheduler_];
  // current_task_scheduler_ = (current_task_scheduler_ + 1) %
  // task_schedulers_.size();
...
bool EventLoop::addTriggerEventForce(
  TriggerEvent callback, std::chrono::milliseconds timeout) {
  Mutex::lock locker(mutex_);
  return task_schedulers_[current_task_scheduler_]->addTriggerEventForce(
    callback, timeout);
}
TimerTaskId EventLoop::addTimer(const TimerTask &timerTask) {
  Mutex::lock locker(mutex_);
